...
// 接收数据包函数（在中断服务函数中调用相关函数来处理接收）
/*void receivePacket(USART_TypeDef *USARTx, uint8_t receivedByte) {
    static ReceiveState state = WAITING_FOR_HEADER;
    ...
    switch (state) {
        ...
        case RECEIVING_DATA:
            ...
            if (index >= dataLength) {
                state = WAITING_FOR_TAIL;
            }
            break;
        case WAITING_FOR_TAIL:
            if (receivedByte == 0xef) {
                // 在这里可以对完整接收到的数据包进行处理
                // 例如：
                if (dataLength >= 8) {
                     temperature = (buffer[0] << 8) | buffer[1];
                     humidity = (buffer[2] << 8) | buffer[3];
                     phValue = (buffer[4] << 8) | buffer[5];
                     waterLevel = (buffer[6] << 8) | buffer[7];
                    // 可以添加更多根据数据包内容进行的操作
                }
            }
...
//// 声明全局变量（其他文件需包含此头文件才能访问）
//extern volatile uint8_t USART3_RX_BUF[RX_BUFFER_SIZE];
//extern volatile uint8_t USART3_RX_CNT;  // 接收计数器
...
//            case 1: // 接收数据
//                USART3_RX_BUF[USART3_RX_CNT++] = data;
//                // 当收到4字节（AA+高+低+BB）时检查帧尾
//                if(USART3_RX_CNT >= 4) { 
//                    if(USART3_RX_BUF[3] == 0xBB) { // 检测帧尾0xBB
//                        USART3_RX_Flag = 1;        // 标记接收完成
//                    }
//                    state = 0; // 无论是否成功，回到初始状态
//                }
//                break;
//        }
