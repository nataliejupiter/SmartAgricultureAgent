...
int main(void)
{
    ...
    // 初始化各个外设
	// USART2_Init(115200);      // 调试 printf 及向上位机发送,注意代码里把frintf重定向注释掉了
	LCD_Init();
	LoRA_Init(LORA_BAUDRATE); // 初始化LoRA
	Adc_Init();
...
//    USART3_Init(9600);        // 接收 PH 等下位机数据
    ...
    {
		LCD_ShowString(10, 10, 128, 20, 24, "DHT11 retry");
        // printf("DHT11 init fail, retry...\r\n");
        delay_ms(1000);
    }
    if (retry == 0)
    {
		LCD_ShowString(10, 20, 128, 20, 24, "DHT11 error!");
        // printf("DHT11 error!\r\n");
		delay_ms(1000);
    }
